...
function initCourse() {
    ...
    $("#btnsalirpantalla").css('cursor', 'pointer');
    $("#btnsalirpantalla").click(function() {
...
function setScale() {
    ...
    $('#progressBar').css("left", "" + (520 * scaleTemplate) + "px");
    $('#progressBar').css("bottom", "" + (18.5* scaleTemplate) + "px");
...
function audios_cargados() {
    ...
    if (elCurseStatus == "unknown" || elCurseStatus == "null" || elCurseStatus == "not attempted") {
...
function bloqueaEvaluacion() {
    ...
    $("#btnNext, #btnPrev, #btnTemario, #btnReload, #btnCerrarTemario").css({
        'cursor': 'default',
        'opacity': '0.4'
    });
}
...
function desbloqueaEvaluacion() {
    ...
    $("#btnPrev, #btnTemario, #btnReload, #btnCerrarTemario").css({
        'cursor': 'pointer',
        'opacity': '1'
    });
}
...
scormA = pipwerks.SCORM;
var APIA = scormA.API.getHandle();

//Escribe suspend data
function setSuspend(info){
    scormA.set("cmi.suspend_data",info);
}

//Obtén suspend data
function getSuspend(){
    return scormA.get("cmi.suspend_data");
}

//Acaba el curso
function finishCurso(){
    scormA.set("cmi.core.exit", "suspend");
    APIA.LMSFinish("");
    parent.window.close();
}
...
//Obtener status 
function getStatus(){
    return scormA.get("cmi.core.lesson_status");
}
...
function setStatus(info){
    ...
    switch(firstChar){
        case "p": scormA.set("cmi.core.lesson_status","passed");
        break;

        case "c": scormA.set("cmi.core.lesson_status","completed");
        break;

        case "f": scormA.set("cmi.core.lesson_status","failed");
        break;

        case "i": scormA.set("cmi.core.lesson_status","incomplete");
        break;

        case "b": scormA.set("cmi.core.lesson_status","browsed");
        break;

        default: alert("error: trying to write status");
        break;
    }
}

//Escribir calificación
function setCalificacion(info){
    scormA.set("cmi.core.score.raw",info);
}

//Leer el nombre del estudiante
function getStudentName(){
    return scormA.get("cmi.core.student_name");
}

//Leer bookmarking
function getBookmarking(){
    return scormA.get("cmi.core.lesson_location");
}

//Escribir bookmarking
function setBookmarking(info){
    scormA.set("cmi.core.lesson_location",info);
}
